...
BTS * RightRotation ( BTS *z) {
    ...
    z-> inaltime = maximum(z->stanga, z->dreapta) + 1;
    y-> inaltime = maximum(y->stanga, y-> dreapta) + 1;
...
BTS* iAVL (BTS * node , Echipa Echipa) {
    ...
    else if (node->val.punc_e == Echipa.punc_e)
    {
        ...
        if (k > -1 && node->dreapta != NULL && Echipa.punc_e > (node->dreapta)->val.punc_e)
            return LeftRotation (node);
             else if (node->val.punc_e == Echipa.punc_e)
                if((strcmp(node->val.nume_echipa, Echipa.nume_echipa)) > 0)
                    return LeftRotation(node);

        if (k > 1 && node->stanga != NULL && Echipa.punc_e > (node->stanga)->val.punc_e)
